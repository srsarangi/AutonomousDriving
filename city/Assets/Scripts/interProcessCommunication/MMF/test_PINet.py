...
def Testing(lane_agent, img):
	test_image = cv2.resize(img, (p.x_size, p.y_size)) / 255.0
	test_image = np.rollaxis(test_image, axis=2, start=0)
	w_ratio = p.x_size * 1.0 / img.shape[1]
	h_ratio = p.y_size* 1.0 / img.shape[0]
	in_x, in_y, ti = test_ori(lane_agent, img, np.array([test_image]), w_ratio, h_ratio,draw_type= 'point',thresh=p.threshold_point)
	#_, _, ti = test(lane_agent, np.array([test_image]), thresh=p.threshold_point)
	return in_x, in_y, ti
