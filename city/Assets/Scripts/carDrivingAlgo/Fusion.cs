...
public class Fusion : TrafficSystemVehicle
{
...
//	private iTween.LoopType             m_loopType             = iTween.LoopType.none;
	...
	public virtual void Awake () 
	{
		...
		m_trafficLightYellowEnterDuration = UnityEngine.Random.Range(0.1f, 0.5f);
...
//		m_randomVelocityOffset = Random.Range(m_randomVelocityOffsetMin, m_randomVelocityOffsetMax);
	...
	public virtual void Start () 
	{
		if(TrafficSystem.Instance)
		{
			m_speedVariation    = UnityEngine.Random.Range(0.0f, TrafficSystem.Instance.m_globalSpeedVariation);
			m_lanePosVariation  = UnityEngine.Random.Range(-TrafficSystem.Instance.m_globalLanePosVariation, TrafficSystem.Instance.m_globalLanePosVariation);
		}
        ...
        TrafficNodes(ref trafficnodelist);
	...
	public void CalculatePathToTake( TrafficSystemNode a_nodeToPathTo )
	{
        ...
        if (m_nodeToPathTo)
        {
            Debug.Log("calculatingcalled");
			m_calculatingPathToTake = true;
            Debug.Log("this is next node"+m_nextNode   );
			PathPlanning();
		}
	}

	private Tuple<TrafficSystemNode, float> popMin(ref List<Tuple<TrafficSystemNode, float>> candidateList) {
		...
		Tuple<TrafficSystemNode, float> ret = candidateList[index];
		candidateList.RemoveAt(index);
		return ret;
	}
     private List<Tuple<TrafficSystemNode,TrafficSystemNode>> trafficnodelist = new List<Tuple<TrafficSystemNode, TrafficSystemNode>>();
    private void TrafficNodes(ref List<Tuple<TrafficSystemNode, TrafficSystemNode>> trafficnodelist)
    {
        Debug.Log("111111");
        trafficnodelist.Clear();
        List<TrafficSystemVehicle> temp = TrafficSystem.Instance.m_spawnedVehicles;

        for (int i = 0; i < temp.Count; i++)
        {
                if (temp[i].tag != "Player")
                {
                    if (trafficnodelist.Count == 0)
                    {
                    trafficnodelist.Add(new Tuple<TrafficSystemNode, TrafficSystemNode>(temp[i].m_nextNode, temp[i].m_nextNode.GetNextNode(temp[i], true)));
                     
                    }
                    else if (trafficnodelist.Count > 0 && trafficnodelist[trafficnodelist.Count-1].Item1 != temp[i].m_nextNode && trafficnodelist[trafficnodelist.Count-1].Item2 != temp[i].m_nextNode.GetNextNode(temp[i], true))
                    {
                    trafficnodelist.Add(new Tuple<TrafficSystemNode, TrafficSystemNode>(temp[i].m_nextNode, temp[i].m_nextNode.GetNextNode(temp[i], true)));
        ...
        Debug.Log("Nodes are"+ trafficnodelist[1].Item1 + "" + trafficnodelist[1].Item2);
        Debug.Log(trafficnodelist[1].Item1.transform.position.x + "  " + trafficnodelist[1].Item1.transform.position.y);
        Debug.Log(trafficnodelist.Count + "vehicles");
    }
       public Boolean isVehicle(TrafficSystemNode a, TrafficSystemNode b)
    {
        //Debug.Log("entered");
        for (int i=0; i< trafficnodelist.Count; i++)
        {
            if (a == trafficnodelist[i].Item1 && b == trafficnodelist[i].Item2)
            {
                Debug.Log("true");
                return true;
            }
	...
	private void PathPlanning()
	{
		...
		List<Tuple<TrafficSystemNode, float>> candidateList = new List<Tuple<TrafficSystemNode, float>>();
		...
		candidateList.Add(new Tuple<TrafficSystemNode, float>(m_nextNode, 0.0f));
		...
		while (candidateList.Count > 0) {
			Tuple<TrafficSystemNode, float> candidate = popMin(ref candidateList);
			...
			foreach (TrafficSystemNode child in children) {
				...
				float g = candidate.Item2 + displacementMagnitude(child, candidate.Item1);
                // f = g + h
                float f;                                    //pk
                if (isVehicle(candidate.Item1, child))
                {
                    Vector3 temp = transform.position - candidate.Item1.transform.position;
                    float temp2 = temp.magnitude;
                    f = g + displacementMagnitude(child, m_nodeToPathTo) + 500/temp2;
                    if (child.Parent.m_speedLimit > 0)
                    {
                        f = g + displacementMagnitude(child, m_nodeToPathTo) + 500/temp2 + displacementMagnitude(candidate.Item1, child)/child.Parent.m_speedLimit;
                    }
                }
                else
                {
                    if (child.Parent.m_speedLimit > 0)
                    {
                        f = g + displacementMagnitude(child, m_nodeToPathTo) + displacementMagnitude(candidate.Item1, child)/child.Parent.m_speedLimit;
                    }
                    else
                        f = g + displacementMagnitude(child, m_nodeToPathTo);
                }
                ...
                if (distanceTillNode.ContainsKey(childId)) {
					if (distanceTillNode[childId] > f) {
                        ...
                        candidateList.Add(new Tuple<TrafficSystemNode, float>(child, f));
					}
				} else {
        ...
        Debug.Log("Path Distance" + fBestYet);
		// backtracking givves the reverse path
		m_nodeToPathToRoute.Reverse();

		m_doneCalculatingPath = true;
		m_calculatingPathToTake = false;
        Debug.Log("Path Calculated with no of nodes " + m_nodeToPathToRoute.Count);
         TestCalculatedPath();
		return;
	}
   

	public void TestCalculatedPath()
	{
		...
		{
            Debug.Log(m_nodeToPathToRoute[nIndex]);
            if (m_nodeToPathToRoute[nIndex] && m_nodeToPathToRoute[nIndex].GetComponent<Renderer>())
            { 
                Line.positionCount = m_nodeToPathToRoute.Count+1;
                Line.startWidth = 0.4f;
                Vector3 temp = new Vector3 (m_nodeToPathToRoute[nIndex].transform.position.x, m_nodeToPathToRoute[nIndex].transform.position.y, m_nodeToPathToRoute[nIndex].transform.position.z);
                Line.SetPosition(nIndex+1, temp);
                m_nodeToPathToRoute[nIndex].Setlayer();
                m_nodeToPathToRoute[nIndex].GetComponent<Renderer>().enabled = true;
 ...
 /*   public void drawpath()
    {
        Debug.Log("lllllllllllllllllllllllllllll");
        Vector3 temp1 = new Vector3(transform.position.x, transform.position.y, transform.position.z);
        Line.SetPosition(0, temp1);
        Line.startWidth = 0.4f;
        int j = 1;
        for (int i = m_nodeToPathToRouteIndex; i < m_nodeToPathToRoute.Count ; i++)
        {
            Vector3 temp = new Vector3(m_nodeToPathToRoute[i].transform.position.x, m_nodeToPathToRoute[i].transform.position.y, m_nodeToPathToRoute[i].transform.position.z);
            Line.SetPosition(j, temp);
            m_nodeToPathToRoute[i].Setlayer();
            j = j + 1;
        }
    }*/
...
//		CanSeeVehicleViaSphere();
        ...
        TrafficSystemVehicle thisVehicle = transform.GetComponent<TrafficSystemVehicle>();
		if(m_nextNode)
		{
			// casting to check car closest to the front
			...
			for(int hIndex = 0; hIndex < hitInfoAll.Length; hIndex++)
			{
				...
				if(vehicle)
				{
					if(vehicle != thisVehicle && vehicle.VehicleHit != thisVehicle)
					{
						StopMoving = true;
						VehicleHit = vehicle;
						// Set velocity of this car equal to car in the front
						m_velocityMax = VehicleHit.m_velocityMax;
						m_waitTimer = 0.0f;
						// Timer for rechecking if this car is close to another
						m_waitTime = UnityEngine.Random.Range(m_waitTimeMin, m_waitTimeMax);
					}
				}
			}
...
//			print ("TrafficLight: " + TrafficLight);
		...
		if(!StopMoving && m_nextNode && !CrashDetected)// && !m_pathingStarted)
		{
			...
			if(VehicleInSights)
			{
				if(VehicleInSights.VehicleInSights && VehicleInSights.VehicleInSights == thisVehicle)
				{
...
//
	...
	protected void SetNextNode( TrafficSystemNode a_node )
	{
		...
		if(m_nodeToPathToRoute.Count > 0 && m_nodeToPathToRouteIndex < m_nodeToPathToRoute.Count)
		{
            ...
            Debug.Log("Node set to " + m_nextNode + "" + m_nodeToPathToRouteIndex);
         /*   if(m_nodeToPathToRouteIndex>0)
            {
                drawpath();
